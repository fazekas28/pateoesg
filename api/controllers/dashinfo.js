...
export const getBarChartInfo = async (req, res) => {
 const { filial, month } = req.body
 const googleSheets = await db();
 const spreadsheetId = "1drKHdk7GW8R24Dv-SlvDbupRuk2nZ-OfxAJfQAWaR94";

 const getBarChartInfo = await googleSheets.spreadsheets.values.get({
  spreadsheetId,
  range: "MTR!A2:E",
 });

 function getFilialCount(data) {
  const filialCount = {};
  for (const item of data) {
   const filial = item[3];
   if (filial in filialCount) {
    filialCount[filial]++;
   } else {
    filialCount[filial] = 1;
   }
  }
  return filialCount;
 }
 ...
 if (!getBarChartInfo.data.values) {
 ...
 if (!filial && !month) {
  const filialCount = getFilialCount(getBarChartInfo.data.values)
  const labels = Object.keys(filialCount);
  const data = Object.values(filialCount);
  res.status(200).json({ labels: labels, data: data })
 }

 if (filial) {
  res.status(200).json({ msg: 'Filial selecionada' })
 }

 if (month) {
  res.status(200).json({ msg: 'mes selecionado!' })
